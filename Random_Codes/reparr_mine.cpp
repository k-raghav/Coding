...
int main() {
    ...
    int n = A.size();
    vector<int> range_count;
    //sqroot = ciel(sqrt(n));
    int box = ceil(sqrt(n-1));
    int range = 0;
    cout << "box is " << box << endl;
    for (int i = 0 ; i < box ; i++) {
        range_count.push_back(0);
    }
    for (int i = 0 ; i < n ; i++ ) {
        float tmp = (float)A[i] / box;
	range = (int)tmp;
	if(ceilf(tmp) == tmp) range--;
        cout << "ai and range " << A[i] << " " << range << endl;
        range_count[range]++;
    }
    cout << n-1 << endl;
    cout << box << endl;
    ...
    cout << endl;
    int repeated_range=0;
    for (int i = 0 ; i < box ; i++) {
        if(range_count[i] > box) repeated_range = i;
    }
    cout << repeated_range << endl;
    int start = (box * repeated_range) + 1;
    int end = box * (repeated_range+1);
    cout << start << " " << end << endl;
    ...
    for (int i = 0 ; i < n ; i++) {
        if(A[i] >= start and A[i] <= end) {
           it = number_count.find(A[i]);
           it->second++;
           if(it->second > 1) cout << "ANS " << it->first << endl;
        }
    }
    cout << "ANS 0" << endl;
}
